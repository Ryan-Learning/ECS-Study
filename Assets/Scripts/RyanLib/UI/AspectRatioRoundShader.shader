Shader "Unlit/AspectRatioRoundShader"
{
	Properties
	{
		...
		[Toggle(UNITY_UI_ALPHACLIP)] _UseUIAlphaClip("Use Alpha Clip", Float) = 0
			...
			_AspectRatio("Aspect Ratio", Float) = 0
		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : COLOR
				{
					...
					if (abs(i.RadiusBuceVU.x) < 0.5 - _RADIUSBUCE) {
						col = tex2D(_MainTex, i.uv) * i.color;
					}
					else if (abs(i.RadiusBuceVU.y) < 0.5 - _RADIUSBUCE) {
						col = tex2D(_MainTex, i.uv) * i.color;
					}
						else if (length(abs(i.RadiusBuceVU) - float2(0.5 - _RADIUSBUCE, 0.5 - _RADIUSBUCE)) < _RADIUSBUCE)
						{
							col = tex2D(_MainTex, i.uv) * i.color;
						}
						else
