...
實體組件系統（ECS）是Unity的一個新的開發模型，將遊戲中的實體（Entity）拆分為組件（Component）和系統（System）。ECS專注於數據導向的設計，可以更好地利用現代硬件的多核心處理能力。與作業系統結合使用，可以獲得極高的性能。

4、ECS + Job System + Burst Compiler（實體組件系統 + 作業系統 + Burst編譯器）：
Burst編譯器是Unity的一個優化工具，可以將C#代碼轉換為高效的本機代碼。與ECS和作業系統結合使用，Burst編譯器可以進一步提高遊戲的性能。
